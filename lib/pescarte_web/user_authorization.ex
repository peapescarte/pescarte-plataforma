defmodule PescarteWeb.UserAuthorization do
  import Plug.Conn
  ...
  defp maybe_halt(_any, conn) do
    ...
    |> Phoenix.Controller.redirect(to: UserAuthentication.signed_in_path())
    |> halt()
  end

  def on_mount(:ensure_admin_role, _params, session, socket) do
    socket = UserAuthentication.mount_current_user(session, socket)
    ...
    if user && permit?(user, :admin) do
      ...
      socket =
        ...
        |> Phoenix.LiveView.redirect(to: UserAuthentication.signed_in_path())
