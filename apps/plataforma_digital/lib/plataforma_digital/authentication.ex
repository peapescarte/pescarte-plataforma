defmodule PlataformaDigital.Authentication do
  @moduledoc """
  ...
  def log_out_user(%Plug.Conn{} = conn) do
    ...
    user_token && CredenciaisHandler.delete_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      PlataformaDigital.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def log_out_user(%Socket{} = socket) do
    ...
    user_token && CredenciaisHandler.delete_session_token(user_token)
    PlataformaDigital.Endpoint.broadcast_from(self(), socket.id, "disconnect", %{user: user})
  end
  ...
  the current_user:
      defmodule PlataformaDigital.PageLive do
        use PlataformaDigital, :live_view
        on_mount {PlataformaDigital.Authentication, :mount_current_user}
        ...
      end
  Or use the `live_session` of your router to invoke the on_mount callback:
      live_session :authenticated, on_mount: [{PlataformaDigital.UserAuth, :ensure_authenticated}] do
