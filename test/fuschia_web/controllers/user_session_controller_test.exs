defmodule FuschiaWeb.UserSessionControllerTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FuschiaWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "POST /acessar" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_fuschia_web_user_remember_me"]
      assert redirected_to(conn) == "/app/usuarios/#{user.id}"
    end
